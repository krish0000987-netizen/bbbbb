...
def login_to_aliceblue():
    try:
        ...
        if session and session.get('stat') == 'Ok':
            ...
            # Try to download contract master, but don't fail if it errors
            try:
                ...
                logger.warning("⚠️ Will try to fetch instruments individually")
...
# ============================================================================
# DATA FETCHER - WITH 5-MINUTE AGGREGATION
# ============================================================================
class DataFetcher:
    ...
    def get_historical_data(self, instrument, from_date, to_date, interval):
        """
        Fetches historical data - returns DataFrame
        """
        try:
            logger.debug(f"Fetching: {from_date} to {to_date}, interval={interval}")

            # Use interval=1 (1-minute candles)
            data = self.alice.get_historical(
                instrument=instrument,
                from_datetime=from_date,
                to_datetime=to_date,
                interval=1,  # Always fetch 1-min data
                indices=False
            )
            ...
            if isinstance(data, pd.DataFrame):
                df = data
                logger.debug(f"✅ Got DataFrame directly. Shape: {df.shape}")
            elif isinstance(data, dict):
                # Handle dict response
                if data.get('stat') == 'Not_Ok':
                    logger.debug(f"API returned Not_Ok: {data.get('emsg')}")
                    return None
                ...
                if 'data' in data:
                    ...
                    logger.debug(f"Unknown dict structure: {data.keys()}")
                    return None
            else:
            ...
            # Find time column
            time_column = None
            for col in ['datetime', 'time', 'timestamp', 't']:
                if col in df.columns:
                    time_column = col
                    break

            if not time_column:
                logger.debug(f"No time column. Columns: {df.columns.tolist()}")
                return None
    ...
    # ✅ NEW FUNCTION: Aggregate 1-min candles to 5-min candles
    def aggregate_to_5min(self, df_1min):
        """
        Convert 1-minute candles to 5-minute candles
        This matches what you see on your trading charts!
        """
        try:
            ...
            df_5min = df_1min.resample('5min').agg({
                'open': 'first',   # First open of the 5-min period
                'high': 'max',     # Highest high in 5 minutes
                'low': 'min',      # Lowest low in 5 minutes
                'close': 'last',   # Last close of the 5-min period
                'volume': 'sum'    # Total volume in 5 minutes
            })
            ...
            logger.debug(f"✅ Aggregated {len(df_1min)} 1-min candles → {len(df_5min)} 5-min candles")
            return df_5min
